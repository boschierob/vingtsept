...
const updateMonthlySheet = async (req, res) => {
    try {
      ...
      // Mettre à jour le document Worker associé
      const workerId = updatedMonthlySheet.related_worker;
      ...
      // Mettre à jour la feuille mensuelle dans le tableau monthlySheets du Worker
      const index = worker.monthlySheets.findIndex(sheet => sheet.equals(monthlySheetId));
      if (index !== -1) {
        worker.monthlySheets[index] = updatedMonthlySheet._id;
        await worker.save();
      }
